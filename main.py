...
class Main:
    ...
    def __init__(self, root):
        ...
        self.btn_recount = tk.Button(self.root, text="add", command=self.create_statement_and_sector, compound=tk.TOP,bd=0)  # button is open recount window
        self.btn_recount.pack(side=tk.LEFT)

        self.btn_goal = tk.Button(self.root, text="goals", command=self.open_goal, compound=tk.TOP)  # button is open goals window
...
class Recount:
    ...
    def __init__(self, root):
        ...
        self.select_box = ttk.Combobox(self.child_root, values=['spending', 'earning'])  # selestion of spending or earning
        ...
        self.btn_exit = tk.Button(self.child_root, text="exit", command=self.child_root.destroy)
        self.btn_exit.place(x=300, y=170)

        self.btn_add = tk.Button(self.child_root, text="apply", command = self.set_text)
